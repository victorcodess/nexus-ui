...
- **Image** — Composable image renderer for URLs, base64, and Uint8Array payloads with preview, loader, lightbox, and action slots
- **Feedback Bar** — Feedback prompt bar with composable prompt, actions, optional tooltips, and close slot for ratings and sentiment
- **Toaster** — Headless toast notifications powered by Sonner, with variant-aware styling and custom action/cancel controls
- **Chain of Thought** — Structured multi-step reasoning timeline with per-step status (`pending`, `active`, `completed`, `error`), collapsible titles and outputs, and optional root auto-close when all steps complete
