...
<Step>
<h3>Install dependencies: <code>streamdown</code>, <code>@hugeicons/react</code>, <code>@hugeicons/core-free-icons</code>, and shadcn <code>collapsible</code> (or use the CLI so they install automatically).</h3>
</Step>
<Step>
...
Use `Reasoning` with [Vercel AI SDK](https://sdk.vercel.ai) by collecting `reasoning` message parts from assistant turns and mapping them to:
...
`sendReasoning` is enabled by default in `toUIMessageStreamResponse`, but this example keeps it explicit.
...
<Step>
<h3>Render assistant reasoning parts with <code>Reasoning</code></h3>
...
Root component. Manages streaming timing, open state behavior, and label context for children.
...
Trigger row with built-in brain icon, label text, and rotating chevron. If `children` is provided, it replaces the default label text.
...
Collapsible content wrapper with enter/exit animation and markdown rendering via `Streamdown`.
