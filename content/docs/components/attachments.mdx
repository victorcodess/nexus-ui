...
Maps a browser **`File`** to **`AttachmentMeta`**. Sets **`type`** from **`file.type`**, and sets **`url`** from **`options.objectUrl`** when provided. The **`Attachments`** picker passes a blob **`url`** for every picked file; tile previews use **`url`** for images and a muted **`video`** element for videos.
