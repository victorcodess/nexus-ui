...
Composable attachment UI for chat and messaging: thumbnails or type icons, remove actions, optional upload progress, and a controlled `Attachments` root that owns the hidden file input and pairs with `AttachmentTrigger`. The demo above uses the default **`compact`** (square) variant; **inline** and **detailed** layouts are shown below.
...
Object URLs created by this picker (**`URL.createObjectURL`** for image, video, and audio **`File`**s) are tracked and **`URL.revokeObjectURL`** when an attachment leaves the list or when **`Attachments`** unmounts. Blob URLs you attach yourself (outside this flow) are not revoked by the component.
...
<TypeTable
  type={{
    ...
    onFilesRejected: {
      ...
      description:
        "Optional. When the user selects files that are not all appended: oversize (`tooLarge`), over `maxFiles` (`overMaxFiles`), or extra files when `multiple` is false (`truncatedByMultiple`).",
    },
    children: {
...
<TypeTable
  type={{
    variant: {
      type: '"compact" | "inline" | "detailed"',
      default: '"compact"',
      description:
        "`compact`: square preview; `inline`: horizontal chip with optional fade for long names; `detailed`: row with metadata.",
    },
    attachment: {
      type: "AttachmentMeta",
...
<TypeTable
  type={{
    ...
    position: {
      ...
      description:
        "Icon position; defaults to `corner` for `compact` and `detailed`, and `center-end` for `inline`.",
    },
    className: {
      type: "string",
...
Maps a browser **`File`** to **`AttachmentMeta`**. Sets **`type`** from **`file.type`**, and sets **`url`** from **`options.objectUrl`** when provided. The **`Attachments`** picker supplies object URLs for image, video, and audio files (video previews use a muted **`video`** element in tile layouts).
