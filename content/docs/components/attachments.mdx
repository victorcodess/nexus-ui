...
For **large pasted plain text** (e.g. from **`appendFiles([file], { paste: true })`** after **`⌘V`** in your prompt), use **`variant="pasted"`** with **`AttachmentMeta.source === "paste"`**. The tile shows a short excerpt and a **Pasted** footer with remove.
