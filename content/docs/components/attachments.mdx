...
`Attachments` wraps the hidden input; keep **`AttachmentTrigger`** (and the list) **inside** the same **`Attachments`** tree as the [Prompt Input](/docs/components/prompt-input) textarea and actions.
...
Build **`FileUIPart`** values from your **`AttachmentMeta`** list and pass them as **`files`** (URLs can be data URLs, HTTPS URLs, or blob URLs your app can read; for production, prefer stable URLs after upload). Wrap **`PromptInputTextarea`**, **`PromptInputActions`**, and the optional **`AttachmentList`** in **`Attachments`** so **`AttachmentTrigger`** stays in context.
