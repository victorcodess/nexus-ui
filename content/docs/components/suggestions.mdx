...
<TypeTable
  type={{
    ...
    highlight: {
      type: "string | string[]",
      description: "Term(s) to render in a lighter weight. Non-matching text is bolded. Matches are case-insensitive.",
    },
    value: {
      type: "string",
...
Full-width panel that displays below the input, covering the category pills. Use with `SuggestionPanelHeader`, `SuggestionPanelTitle`, `SuggestionPanelClose`, and `SuggestionPanelContent`.
...
Close button for the panel. Renders as a native button.
...
Content wrapper for the panel's suggestion list. Use with `Suggestions` and `SuggestionList` inside.
