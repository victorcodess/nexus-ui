...
const INLINE_CITATION_URL_PREFIX = "https://citations.nexus.local/";

function withInlineCitationLinks(text: string) {
...
function parseCitationIds(href: string) {
...
function citationSourcesFromIds(
  ids: number[],
  sourceUrls: ReturnType<typeof sourceUrlPartsFromMessage>,
) {
  const uniqueIds = Array.from(new Set(ids));
  return uniqueIds
    .map((id) => sourceUrls[id - 1])
    .filter((source): source is (typeof sourceUrls)[number] => Boolean(source))
    .map((source) => ({
      url: source.url,
                          ...
                          <MessageMarkdown
                            ...
                            components={{
                              a: ({ href, children, ...props }: any) => {
                                ...
                                const ids = parseCitationIds(href);
                                if (ids.length === 0) {
                                  return React.createElement(
                                    "a",
                                    { ...props, href } as any,
                                ...
                                const citations = citationSourcesFromIds(
                                  ids,
                                  sourceUrls,
                                );
